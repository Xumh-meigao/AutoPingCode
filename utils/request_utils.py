        ...
        if response and response.status >= 400:
            try:
                ...
                if len(response_text) > max_length:
                    response_text = response_text[:max_length] + "... (truncated)"
                
                logger.error(f"Response Body (Status {response.status}):")
...
class RetryableRequest:
    ...
    def request(self, method, url, **kwargs):
        ...
        try:
            ...
            # 记录详细的错误信息
            logger.error(f"Request failed after retries")
            ...
            if 'headers' in kwargs:
                logger.error(f"  Headers: {kwargs['headers']}")
            if 'params' in kwargs:
                logger.error(f"  Params: {kwargs['params']}")
            if 'json' in kwargs:
                logger.error(f"  JSON Data: {kwargs['json']}")
            if 'data' in kwargs:
                logger.error(f"  Form Data: {kwargs['data']}")
            
            # 如果有响应对象，记录响应信息
            if hasattr(e, 'response') and e.response is not None:
                logger.error(f"  Response Status: {e.response.status_code}")
                logger.error(f"  Response Body: {e.response.text[:500]}")  # 只记录前500字符
            
            raise Exception(f"Request failed after retries: {str(e)}") from e
    ...
    def post_request_json(self, url: str, json=None, **kwargs):
        try:
            ...
            logger.error(f"post_request_json failed")
            logger.error(f"  URL: {url}")
            logger.error(f"  JSON Data: {json}")
            logger.error(f"  Error Type: {e.__class__.__name__}")
            logger.error(f"  Error Message: {str(e)}")
            # 注意：详细的请求错误已经在 request 方法中记录过了
            return None
